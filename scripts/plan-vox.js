...
function heuristicDesign(text, i, total) {
  ...
  else if (/(action step|takeaway|checklist|habit|rule\s+#?\d|lesson\s+#?\d)/i.test(text) && li) type = "checklist";
  else if (li) type = "list";
  else if (/(photograph|portrait|case study|snapshot|archival photo)/i.test(text)) type = "polaroid";
  else if (/(compound|exponential|growth curve|trajectory|compounding|curve of)/i.test(text)) type = "chart";
  else if (timelineStops(text)) type = "timeline";
  else if (/(\$?\d[\d,\.]*\s?(%|percent|million|billion|trillion|x\b))/i.test(text)) type = "stat";
...
(async () => {
  ...
  const beats = rawBeats.map((b, i) => {
    ...
    } else if (d.type === "checklist" || (/(action step|takeaway|checklist|habit|rule\s+#?\d|lesson\s+#?\d)/i.test(texts[i]) && (items.length >= 2 || textList.length >= 2))) {
      type = "checklist";
      props.checklistItems = items.length >= 2 ? items : textList;
    } else if (items.length >= 2 || textList.length >= 2) {
      type = "list";
      if (items.length < 2) items = textList;
    } else if (d.type === "stat" || /(\$?\d[\d,\.]*\s?(%|percent|million|billion|trillion|x\b))/i.test(texts[i])) type = "stat";
    else if (d.type === "quote") type = "quote";
    else if (d.type === "polaroid" || (/(photograph|portrait|case study|snapshot|archival photo)/i.test(texts[i]) && d.image)) {
      type = "polaroid";
    } else if (d.type === "chart" || /(compound|exponential|growth curve|trajectory|compounding|return on investment)/i.test(texts[i])) {
      type = "chart";
    }
